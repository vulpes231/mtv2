...
export const getAcctTrnxs = createAsyncThunk(
  "trnx/getAcctTrnxs",
  async (queryData, { rejectWithValue }) => {
    const { filterBy, filterValue, limit, sortBy, page, accountId } = queryData;

    try {
      const url = `/transactions/${accountId}/?filterBy=${filterBy}&filterValue=${filterValue}&page=${page}&limit=${limit}&sortBy=${sortBy}`;
